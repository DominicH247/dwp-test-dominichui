  ...
  test("GET, responds with status code 200 and an array of user object of people living within 50 miles of the city of london", () => {
    return request
      ...
      .then(({ body: users }) => {
        ...
        expect(users.london_users.usersInLondonByCoords.length === 3).toBe(
          true
        );
        expect(Array.isArray(users.london_users.usersListedInLondon)).toBe(
          true
        );
...
describe("Handle invalid methods, POST, PATCH/PUT, DELETE", () => {
  test("Responds with 405 and message of invalid method", () => {
    return request
